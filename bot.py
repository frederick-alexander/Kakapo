...
from const import discord_token as token
...
@bot.command(name = "updateMission", help = "Clears all slots and updates the mission details")
async def updateMission(ctx, mission_id: int, date: str, time: str):

	#Connect to the database
	conn = sqlite3.connect("slotlist.db")
	cursor = conn.cursor()

	# Check if requested mission_id exists
	sql = 'SELECT * FROM "{}"'.format(mission_id)
	cursor.execute(sql)
	if not cursor.fetchone():

		# If requested mission is not found, tell the user this"
		await ctx.send('{} Your mission ID {} was not found'.format(ctx.author.mention, str(mission_id)))

	else:

		# If requested mission is found, update the date and time of mission
		date = date + " " + time
		sql = 'UPDATE "?" SET mission_date = (?) WHERE mission_id = "?"'#.format(mission_id, date, mission_id)
		cursor.execute(sql, [mission_id, date, mission_id])
		conn.commit()

		# Update slotlist on Discord with the updated information
		missioninfofnc(ctx, mission_id, True)

		# Notify the user that the mission has been updated
		sql = 'SELECT mission_name FROM missions WHERE mission_id = "{}"'.format(mission_id)
		cursor.execute(sql)
		await ctx.send("{} Your mission {} was updated.".format(ctx.author.mention, str(c.fetchone()[0])))
